...
export class TeachersDay {
  constructor(private zone: NgZone) {
    ...
    window.ng.Component = {
      ...
      Zone: this.zone,
      Property: (...args) => {
        if (args[1]) return (this[args[0]] = args[1]);
        return this[args[0]];
      },
  ...
  Add(Row: Row) {
    ...
    var TableLength = this.dataSource.data.push(
      Object.assign(Row ? Row : {}, { Rank: this.Rank })
    );
    this.Rank++;
  ...
  Remove(Name) {
    this.dataSource.data.splice(
      this.dataSource.data.findIndex(Row => Row["Name"] === Name),
      1
    );
    this.dataSource._updateChangeSubscription();
  }
